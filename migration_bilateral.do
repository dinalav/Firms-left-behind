...
// download the dataset // Note, the dataset can be updated --> need to modify the code
...
replace ind = "`x'" if strpos(label, "`x'")
}
foreach y in nationality birth {
replace ind = ind + "_by_" + "`y'" if strpos(label, "`y'")
}
tab ind
drop VAR label
...
sum ch
replace population_by_birth = ch*population_by_nation if mi(population_by_birth)

ren time year
keep dest* origin* year population_by_birth
destring year, replace
ren population population_by_birth_oecd
...
replace population_by_birth = ch*population_by_nation if mi(population_by_birth)
drop ch
drop population_by_nation
save ESTAT_data, replace
...
keep if inrange(year, 2000, 2018)
do "$codes/help_codes/country_to_region"
keep if inrange(origin_region, 2,3)
keep if dest_r == 1 | dest_r == 4
tab origin
tab dest

merge 1:1 dest origin year using ESTAT_data
drop _merge

merge 1:1 dest origin year using OECD_data
drop _merge 

pwcorr population*
sum population*
keep if inrange(year, 2000, 2018)
drop if origin == "CY" | origin == "MT"

gen immigrants_by_birth = population_by_birth_ESTAT
replace immigrants = population_by_birth_oecd if mi(immigrants)
...
save bilateral_migration_data, replace
...
// aggregate construction and some other service sectors
...
collapse (sum) value, by(year nace2 dest origin)
foreach x of varlist value {
replace `x' = . if `x' == 0 // collapsing puts missing to zeros
}
...
// merge with bilateral migration data 
merge m:1 dest origin year using bilateral_migration_data
drop if _merge == 2
drop _merge
...
save "$temp/Dataset_ifmissing", replace
